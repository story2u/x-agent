...
flowchart LR
  ...
  Agent --> PiAI["pi-ai openai-codex / openai"]
  PiAI --> Model["ChatGPT Plus/Pro · gpt-5.5-codex"]
```
...
- Model：pi-ai `openai-codex`（默认）或 `openai`；凭据来自 `OPENAI_CODEX_OAUTH_CREDENTIALS` / `OPENAI_CODEX_ACCESS_TOKEN`，或 `OPENAI_API_KEY` fallback。
...
- `src/lib/skills/local-skills.ts`：本地 skill loader、自动选择、prompt compiler。
...
- `src/lib/pi-credentials.ts`：模型凭据选择、pi-ai OAuth refresh、凭据轮转告警。
...
MVP agent 只生成文本 artifact：`tweet`、`hashtags`、`rationale`、`safetyNotes`、`dailyFortune?`。`TwitterCreative.media` 是保留扩展位，不在 TUI 暴露。

生成流程：

1. `resolveRuntimeSkill` 选择本地 skill（手动 > fortune 关键词自动 > 默认）。
2. 加载完整 `SKILL.md` 并编译进 skill-aware prompt。
...
无效（validation errors）的 SKILL.md 不会被自动选择，也不会注入 prompt。
