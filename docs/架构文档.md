...
无效（validation errors）的 SKILL.md 不会被自动选择，也不会注入 prompt。References 会按文件名排序并以 `always` 策略加载到 prompt；`RunSkillTrace.loadedReferences` 会列出实际加载的 SKILL.md 和 references。`daily-fortune-tweet` 的 references 既有文案/运营/安全资料，也包含星座命理底料（`astrology-signs.md`、`astrology-daily-engine.md`）。

质量门分两层：`scripts/eval-skills.ts` / `npm run eval:skills` 只校验 eval spec 的形状；`scripts/eval-fortune-run.ts` / `npm run eval:fortune` 会**真跑** fortune pipeline，并用确定性规则 + 独立 LLM-judge 对真实输出评分、打印运营达标率（需模型凭据，本地手动门）。
