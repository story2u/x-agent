...
- 实时进度：生成时订阅 `generateTwitterCreative(..., { onProgress })`，打印 pipeline/stage progress、可见模型 `text_delta`、tool capture 和 token usage；这些事件只显示在终端，不落地。
...
1. `resolveRuntimeSkill` 选择本地 skill（手动 > fortune 关键词自动 > 默认），并可通过 progress callback 通知 TUI。
   - 若选中 `daily-fortune-tweet`，改走 `runFortunePipeline`（`src/lib/fortune/pipeline.ts`，5 段独立推理 + 注入确定性当日星象事实 + 实时 stage/text_delta 事件，详见 `docs/数据流程.md`）；下面 2-6 是其余 skill 的单次路径。
