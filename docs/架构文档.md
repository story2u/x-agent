...
- `src/lib/types.ts`：共享请求/响应类型，以及 TUI 订阅的 `GenerateProgressEvent` / `GenerateProgressOptions`。
- `src/lib/fortune/request-overrides.ts`：Daily Fortune 自然语言请求上下文归一化（受众/语气/输出类型默认值与显式覆盖）。
...
- `/debug on|off`、`/details`：切换或临时查看内部 artifact 详情；默认 public mode 只展示可发布正文和 hashtags。
- 输入历史：readline 历史跨 TUI 会话持久化到 `.x-agent-tui-history`（gitignored），只保存用户输入行，不保存模型输出。
- 实时进度：生成时订阅 `generateTwitterCreative(..., { onProgress })`；默认 public mode 不打印 pipeline/stage/tool/model delta 细节，只显示 effective context、可发布正文和 hashtags。`/debug on` 后打印 pipeline/stage progress、可见模型 `text_delta`、tool capture 和 token usage。这些事件只显示在终端，不落地。
...
1. TUI 先用 `deriveFortuneRequestOverrides()` 归一化 fortune 请求上下文，再由 `resolveRuntimeSkill` 选择本地 skill（手动 > fortune 关键词自动 > 默认），并可通过 progress callback 通知 TUI。
   - 若选中 `daily-fortune-tweet`，改走 `runFortunePipeline`（`src/lib/fortune/pipeline.ts`，5 段独立推理 + 注入确定性当日星象事实 + stage/text_delta 事件；这些事件默认只在 debug mode 打印，详见 `docs/数据流程.md`）；下面 2-6 是其余 skill 的单次路径。
...
Daily Fortune final 组装前会运行 `normalizeLongTweetOpening()`，避免 `今日财运关键词：X` 这类模板化第一行进入 public long post（即使模型输出的关键词和 spine keyword 不完全一致也会处理）；`validatePublicPostSurface()` 同时检查内部审核话术和非技术受众技术黑话。
