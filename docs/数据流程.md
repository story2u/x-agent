...
- **provenance**：`astroFactors()` 给每个变量打 `sourceLevel`（deterministic-calendar / traditional-symbolic / approximate-astronomical / creative-rotation）+ `confidence`；`formatAstroDayBlock` 注入 prompt 时标注来源，模型不把创意种子当命理事实（类型见 `src/lib/fortune/types.ts`）。
