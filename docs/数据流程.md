...
- **FortuneContext**：`resolveFortuneContext(input)`（`src/lib/fortune/context.ts`）把 western / eastern（Slice 3 占位）/ seth / creative 四层合并为一个带 provenance 的对象，`formatFortuneContextForPrompt` 注入各 stage（取代散落的 astroBlock）；经 `GenerateResponse.fortuneContext` 暴露给 TUI `/context`。
